...
: Node("distortion_corrector_node", options)
{
  ...
  // Subscriber
  velocity_report_sub_ = this->create_subscription<tier4_vehicle_msgs::msg::VelocityReport>(
...
void DistortionCorrectorComponent::onVelocityReport(
  const tier4_vehicle_msgs::msg::VelocityReport::ConstSharedPtr velocity_report_msg)
{
...
bool DistortionCorrectorComponent::undistortPointCloud(
  const std::deque<tier4_vehicle_msgs::msg::VelocityReport> & velocity_report_queue_,
  const tf2::Transform & tf2_base_link_to_sensor, PointCloud2 & points)
{
  ...
  auto velocity_report_it = std::lower_bound(
    ...
    first_point_time_stamp_sec, [](const tier4_vehicle_msgs::msg::VelocityReport & x, const double t) {
