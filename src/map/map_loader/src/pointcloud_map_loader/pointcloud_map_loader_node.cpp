...
: Node("pointcloud_map_loader", options)
{
  ...
  //   declare_parameter("pointcloud_map_path", std::vector<std::string>({}));
  ...
  //   }
  // }
  const auto pointcloud_map_path = declare_parameter("pointcloud_map_path", "");
  if (isPcdFile(pointcloud_map_path)) {
  pcd_paths.push_back(pointcloud_map_path);
  }
  ...
  if (pcd.width == 0) {
    ...
    RCLCPP_INFO(get_logger(), "Pointcloud Map is loaded.");
  }
